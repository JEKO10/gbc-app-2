...
import { ensureOrderChannel } from "@/notifications/orderNotifications";
...
const DashboardScreen = () => {
  ...
  const registerDeviceToken = useCallback(
    async (deviceToken: string) => {
      ...
      if (pushTokenRef.current === deviceToken) {
        return;
      }

      try {
        const response = await fetch(
          "https://www.gbcanteen.com/api/notification-tokens",
          {
            method: "POST",
            headers: {
  ...
  useEffect(() => {
    if (Platform.OS === "web" || !authToken || !restaurantId) {
      if (!authToken) {
  ...
  }, [authToken, restaurantId, registerDeviceToken]);
